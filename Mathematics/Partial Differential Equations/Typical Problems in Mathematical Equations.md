...
在方法选取上，本次考试考查积分变换法和分离变量法。典型的积分变换法的使用情景如下：
...
|波动方程|圆形| $\theta,t$ | $\theta$ |三角函数（简并）| $t$ |三角函数|
...
|耗散方程|圆形| $r,\theta,t$ | $r,\theta$ |作为本征值的Bessel函数<br>与<br>三角函数（简并）| $t$ |指数衰减|
|耗散方程|球形| $r,\theta,\phi,t$ | $r,\theta,\phi$ |作为本征值的Bessel球函数<br>与<br> $P_l^m$ <br>与<br>三角函数（简并）| $t$ |指数衰减|
